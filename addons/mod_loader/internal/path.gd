...
static func get_mod_paths_from_all_sources() -> Array[String]:
	...
	# Restored - was previously commented out, disabling unpacked mod loading entirely.
	# See TGM-6: this is required for loading mods (e.g. Endless Mode) from
	# "res://mods-unpacked" during development, per the Mod Sources doc comment
	# on ModLoaderOptionsProfile.load_from_unpacked.
